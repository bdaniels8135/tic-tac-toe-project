...
function createGameboard() {
  const Gameboard = (function () {
    ...
    const getCellContent = (rowIndex, colIndex) =>
      _cellContents[rowIndex][colIndex];

    const getRowContent = (rowIndex) => _cellContents[rowIndex];

    const getColumnContent = (colIndex) =>
      _cellContents.map((row) => row[colIndex]);

    const getMajorDiagonalContent = () =>
      _cellContents.map((row) => row[_cellContents.indexOf(row)]);

    const getMinorDiagonalContent = () =>
      _cellContents.map((row) => row[2 - _cellContents.indexOf(row)]);
...
function createGame(playerOne, playerTwo, gameboard) {
  const Game = (function () {
    ...
    const _checkWinDirectionContent = (winDirection) =>
      winDirection.every((cell) => cell === "X") ||
      winDirection.every((cell) => cell === "O");
    ...
    const checkLegalMove = (rowIndex, colIndex) =>
      !gameboard.getCellContent(rowIndex, colIndex);
...
function createDisplayController() {
  const DisplayController = (function () {
    ...
    const updateCellContent = (mark, rowIndex, colIndex) => {
      UI.GAME_CELLS[3 * rowIndex + colIndex].innerHTML = mark;
    };
...
const GameController = (function () {
  ...
  const _endGame = (winnersMark) => {
    ...
    _DC.UI.GAME_CELLS.forEach((gameCell) =>
  ...
  const _takeTurn = (rowIndex, colIndex) => {
    ...
    _DC.updateCellContent(
      _GB.getCellContent(rowIndex, colIndex),
      rowIndex,
      colIndex,
    );
    const [isGameOver, winnersMark] = _GAME.checkGameOver();
    if (isGameOver) _endGame(winnersMark);
    else if (_GAME.getActivePlayer().getType() === "human")
      _DC.updateAnnouncementText(
        `It is ${_GAME.getActivePlayer().getMark()}'s turn.`,
      );
  ...
  const _resolveGameCellClick = (event) => {
    ...
    if (_GAME.checkLegalMove(cellRowIndex, cellColIndex))
      _takeTurn(cellRowIndex, cellColIndex);
  };
  ...
  const _activateGame = () => {
    ...
    else if (playerOneType === "master")
      _takeTurn(..._masterFirstMoves[_getRandomInt(4)]);
